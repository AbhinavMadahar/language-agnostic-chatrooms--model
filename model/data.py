...
def tensors_from_pairs_file(file: Iterable[str], vocab: Vocabulary) -> Generator[Tuple[torch.Tensor, torch.Tensor], None, None]:
    """
    ...
    while True:
        try:
            ...
            break 
        tensor_1 = vocab.sparsely_encoded(sentence_1)
        tensor_2 = vocab.sparsely_encoded(sentence_2)
        yield (tensor_1, tensor_2)
    
    file.close()


def read(languages: List[str], vocab: Vocabulary) -> Dict[Tuple[str, str], Generator[Tuple[torch.Tensor, torch.Tensor], None, None]]:
    """
